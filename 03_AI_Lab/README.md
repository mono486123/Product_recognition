...
* **12/25**:在Roboflow改標籤記得excel線下也要同步改，要不然匯出nc匯錯亂。(更正，不用改，直接全部重新download最好，不會比對混亂，只是excel_id不對而已)。
* **12/28**:標註資料需要更專心或者想辦法，因為資料太多可能要用更快與一致性的方式進行tag，使用Calculate_id.py進行項目多寡確認以免單一品項過多或是過少，test_and_augment.py進行動態黃金比例辨識項目品種(使Rcall、Precision達到一致性85%以上)。
* **12/28**:先直接執行手機檔案安裝(目前評估為Precision: 90.96% | Recall: 36.93%)
* **12/30**:目前進行冰箱飲料識別方案(替代方案)。
* **12/30**:完成及確認準度可行(precision_80%以上)。
