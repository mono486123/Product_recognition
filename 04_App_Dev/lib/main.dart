  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      //await _checkAccess(); // 先驗證，成功後才會跑下面的 initApp
      _initApp();
    });
  }

  Future<void> _initApp() async {
    await _loadProductData();    // 監聽雲端資料庫
  ...
  Future<void> _checkAccess() async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text("系統登入"),
        content: TextField(
          obscureText: true,
          decoration: const InputDecoration(labelText: "輸入管理密碼"),
          onChanged: (v) => input = v,
        ),
        actions: [
          TextButton(
            onPressed: () {
              if (input == "yusan2202") {
                Navigator.pop(context); // 密碼正確，進入系統
              } else {
                SystemNavigator.pop(); // 密碼錯誤，直接關閉 App
              }
            },
            child: const Text("登入"),
          ),
      ...
      FirebaseFirestore.instance.collection('products').snapshots().listen((snapshot) {
        ...
        setState(() {
          ...
          for (var doc in snapshot.docs) {
            ...
            productCategoryMap[id] = (data['category'] ?? "food").toLowerCase().trim();
          }
          _isDataLoaded = true;
        });
      });
    } catch (e) {
      debugPrint("❌ Firebase 載入失敗: $e");
    }
  }
  ...
  Future<void> _takePhotoAndAIProcess() async {
    ...
    try {
      ...
      if (results.isNotEmpty) {
        ...
        _showSimpleSnackBar("AI 未能辨識商品，請再試一次");
      }
    } catch (e) {
      debugPrint("❌ AI 辨識發生錯誤: $e");
      _showSimpleSnackBar("辨識失敗，請檢查相機權限");
    } finally {
      // 5. 無論成功或失敗，都要關閉處理中遮罩
